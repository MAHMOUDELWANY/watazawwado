-- ============================================================================
-- Migration: Teacher Lifecycle RPC Privilege & Transition Hardening
-- Description: Revokes authenticated access to prevent arbitrary execution,
--              and strictly enforces the 'confirmed'/'rescheduled' starting state.
-- ============================================================================
...
BEGIN
    ...
    -- 5. Transition Rule Validation (Strict Contract)
    IF v_booking.status NOT IN ('confirmed', 'rescheduled') THEN
        RAISE EXCEPTION 'Cannot record outcome for booking in % status.', v_booking.status USING ERRCODE = 'P0004';
    END IF;
...
-- Secure the RPC: Revoke from PUBLIC and authenticated, grant ONLY to service_role
